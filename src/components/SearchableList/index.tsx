...
import { Button, Container, FormControlLabel, List, ListItem, Box } from '@mui/material'
...
interface CallbackType {(list: string[]): void }
...
export default function SearchableList({
    ...
    return (
        <Container>
            <StyledList>
                {data.map((item: any) => {
                    const isChecked: boolean = selectionList.includes(item.name) ? true : false
            ...
            <Box display='flex' justifyContent='flex-end' marginTop={'15px'}>
                <Button onClick={() => returnSelectedOptions(selectionList)} variant='contained'>Done</Button>
            </Box>
        </Container>
  )
}
